...
import Horizontal_Nav_Menu from "../components/horizontal_menu_bar";
...
import Fn_Kpi_Actual_Add from "../pages/master/kpiActual";
...
import FnActivityCalendar from "../components/ActivityCalendar";
...
let NavIcons = {
  ...
  2: { Icon: <VscOrganization className="icon " /> },
...
const FnPrivateRoute = ({ children, ...rest }) => {
  ...
  const get_kpi_pending = async () => {
    let kpipending = await fetch(
    ...
    let data = await kpipending.json();
    if (kpipending.status == 200) {
      fnOrderdPending(data);
    }
  };
              ...
              {menudetails.map((item, index) => {
                return item.childNodes.length > 0 ? (
                  <div className="sc_cl_nav_group" key={index}>
                    ...
                    >
                      <i className="nav-item-icon">{NavIcons[item.menu_id].Icon}</i>
