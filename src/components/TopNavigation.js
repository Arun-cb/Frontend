...
const TopNavBar = () => {
  ...
  const kpiPending = async () => {
    let res = await fetch(
      `${process.env.REACT_APP_SERVER_URL}/api/get_kpi_pending_actions/${user.user_id}/`,
      {
        method: "POST",
        headers: {
    ...
    if (res.status === 200) {
      if (data.length > 0) {
        // setPending(data);
        setAlertClose(data[0].show_flag);
        setClose(true);
      }
    }
  };
  ...
  const fnGetConfigCodesDetails = async () => {
    let res = await fetch(
      `${process.env.REACT_APP_SERVER_URL}/api/get_range_config_codes/${0}/${5}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    let data = await res.json();
    if (res.status === 200) {
      if (data.csv_data.filter((data) => data.config_type == "Application Title" && data.is_active).length > 0) {
        let Application_title = data.csv_data
          .filter(
            (data) =>
              data.config_type === "Application Title" && data.is_active
          )
          .map((temp) => temp.config_value);
        setAppname(
          Application_title.length > 0
            ? Application_title
            : "Balanced Scorecard"
        );
      } else {
        setAppname("Application Title");
      }
    }
  };
  ...
  const CloseAlert = async () => {
    await fetch(
      ...
      {
        method: "PUT",
        body: JSON.stringify({ show_flag: "N" }),
        headers: {
          "Content-Type": "application/json",
          Authorization: "Bearer " + String(authTokens.access),
        },
        ...
        <div className="col-6 col-lg-6 d-flex justify-content-end me-2">
          <Col className="align-items-center col-12 col-lg-6 d-flex justify-content-end">
            ...
            <div className="sc_cl_setting_btn" id="top_nav_setting_icon">
              <Link to="/user_profile/24/2" aria-label="Settings">
